...
public class TaskResource {
    ...
    public Response filtrarTarefas(
        ...
        String emailUsuario = context.getUserPrincipal().getName();
        return Response.ok(taskService.filtrarTarefas(status, dataInicial, dataFinal, usuario, emailUsuario)).build();
    }
    ...
    public Response buscarPorId(@PathParam("id") Long id, @Context SecurityContext context) {
        String emailUsuario = context.getUserPrincipal().getName();
        return Response.ok(taskService.getTask(id, emailUsuario)).build();
    }
}
